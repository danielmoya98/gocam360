...
function ensureBrainPackageIsInstalled(tree) {
    ...
    const { install } = (0, devkit_1.getPackageManagerCommand)();
    devkit_1.logger.warn(`The @spartan-ng/brain package has been added to your dependencies. Please run '${install}' to install the package.`);
}
...
function replaceUsages(tree, oldPackageName, newPackageName) {
    (0, devkit_1.visitNotIgnoredFiles)(tree, '.', (path) => {
        ...
        try {
            ...
            devkit_1.logger.warn(`Could not replace ${oldPackageName} with ${newPackageName} in ${path}.`);
        }
    });
}
