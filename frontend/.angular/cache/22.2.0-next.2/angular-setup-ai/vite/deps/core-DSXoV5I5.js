...
import { Wn as Subject, fn as map, rr as Subscription } from "./zipWith-BIPy9JuQ.js";
...
function getParentInjectorIndex(parentLocation) {
	if (ngDevMode) {
		...
		assertGreaterThan(parentInjectorIndex, 27, "Parent injector must be pointing past HEADER_OFFSET.");
	}
...
function unsupportedProjectionOfDomNodes(rNode) {
	const message = `During serialization, Angular detected DOM nodes that were created outside of Angular context and provided as projectable nodes (likely via \`ViewContainerRef.createComponent\` or \`createComponent\` APIs). Hydration is not supported for such cases, consider refactoring the code to avoid this pattern or using \`ngSkipHydration\` on the host element of the component.
...
function invalidSkipHydrationHost(rNode) {
	const message = `The \`ngSkipHydration\` flag is applied on a node that doesn't act as a component host. Hydration can be skipped only on per-component basis.
...
function locateRNodeByPath(path, lView) {
	...
	if (referenceNode === REFERENCE_NODE_HOST) ref = lView[15][0];
		...
		ref = unwrapRNode(lView[parentElementId + 27]);
	}
...
function createRootTView(rootSelectorOrNode, componentDef, componentBindings, directives) {
	const tAttributes = rootSelectorOrNode ? ["ng-version", "22.2.0-next.1"] : extractAttrsAndClassesFromSelector(componentDef.selectors[0]);
...
function getPrimaryBlockTNode(tView, tDetails) {
	const adjustedIndex = tDetails.primaryTmplIndex + 27;
...
function applyDeferBlockState(newState, lDetails, lContainer, tNode, hostLView) {
	...
	if (stateTmplIndex !== null) {
		lDetails[1] = newState;
		const hostTView = hostLView[1];
		const adjustedIndex = stateTmplIndex + 27;
...
function getProviderImportPaths(providerImportsContainer) {
	...
	const visitor = walkProviderTreeToDiscoverImportPaths(providerToPath, /* @__PURE__ */ new Set());
...
function getBaseDocUrl() {
	...
	return `https://${full.includes("-next") || full.includes("-rc") || full === "22.2.0-next.1" ? "next" : `v${VERSION.major}`}.angular.dev`;
}
	...
	tDetails.loadingPromise = Promise.allSettled(dependenciesFn()).then((results) => {
		...
		if (failed) {
			...
			if (directiveDefs.length > 0) {
				...
				tDetails.providers = internalImportProvidersFrom(false, ...directiveTypes);
			}
...
function ɵɵreference(index) {
	...
	return load(contextLView, 27 + index);
}
...
function ɵɵreadContextLet(index) {
	const contextLView = getContextLView();
	const value = load(contextLView, 27 + index);
