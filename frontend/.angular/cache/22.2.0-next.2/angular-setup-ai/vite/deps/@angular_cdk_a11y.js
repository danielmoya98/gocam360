import { Bi as signal, Gr as isSignal, I as EventEmitter, P as EnvironmentInjector, T as DOCUMENT, U as InjectionToken, W as Injector, dt as SecurityContext, et as NgZone, rt as PLATFORM_ID, sa as ɵɵdefineInjector, x as CSP_NONCE, xr as inject, y as APP_ID, yn as effect } from "./_resource-chunk-BEIfFfw3.js";
import { Ba as ɵɵdefineComponent, Gn as RendererFactory2, Ha as ɵɵdefineNgModule, Hn as QueryList, Ji as ɵɵNgOnChangesFeature, Jt as Component, M as createComponent, Nn as NgModule, O as booleanAttribute, Sn as Input, Va as ɵɵdefineDirective, Vt as ApplicationRef, Wa as ɵɵdefineService, Zn as Service, fn as ElementRef, ki as setClassMetadata, pr as ViewEncapsulation, un as Directive, vr as afterNextRender, zn as Output } from "./core-DSXoV5I5.js";
import { E as skip, Et as take, Ot as debounceTime, Qt as filter, S as startWith, Sn as of, Un as BehaviorSubject, Wn as Subject, Yn as Observable, _ as takeUntil, cn as combineLatest, fn as map, h as tap, in as concat, rr as Subscription, yt as distinctUntilChanged } from "./zipWith-BIPy9JuQ.js";
import { y as isObservable } from "./esm5-CHmRCD0e.js";
import { u as isPlatformBrowser } from "./common-BTKkND17.js";
import { r as DomSanitizer } from "./platform-browser-Hp5hRW8q.js";
...
var BreakpointObserver = class BreakpointObserver {
	...
	observe(value) {
		const observables = splitQueries(coerceArray(value)).map((query) => this._registerQuery(query).observable);
...
//#endregion
//#region node_modules/@angular/cdk/fesm2022/coercion-private.mjs
function coerceObservable(data) {
	if (!isObservable(data)) return of(data);
	return data;
}
//#endregion
//#region node_modules/@angular/cdk/fesm2022/_tree-key-manager-chunk.mjs
var TreeKeyManager = class {
	...
	_initializeFocus() {
		...
		let activeIndex = 0;
		for (let i = 0; i < this._items.length; i++) if (!this._skipPredicateFn(this._items[i]) && !this._isItemDisabled(this._items[i])) {
			activeIndex = i;
			break;
		}
		...
		} else this.focusItem(activeIndex);
	...
	constructor(items, config) {
		...
		} else if (isObservable(items)) items.subscribe((newItems) => this._itemsChanged(newItems));
		else {
			this._items = items;
			this._initializeFocus();
		}
		if (typeof config.shouldActivationFollowFocus === "boolean") this._shouldActivationFollowFocus = config.shouldActivationFollowFocus;
		if (config.horizontalOrientation) this._horizontalOrientation = config.horizontalOrientation;
		if (config.skipPredicate) this._skipPredicateFn = config.skipPredicate;
		if (config.trackBy) this._trackByFn = config.trackBy;
		if (typeof config.typeAheadDebounceInterval !== "undefined") this._setTypeAhead(config.typeAheadDebounceInterval);
	}
	change = new Subject();
	destroy() {
		this._typeaheadSubscription.unsubscribe();
	...
	focusItem(itemOrIndex, options = {}) {
		...
		if (this._activeItem !== null && this._trackByFn(activeItem) === this._trackByFn(this._activeItem)) return;
		const previousActiveItem = this._activeItem;
		...
		if (options.emitChangeEvent) this.change.next(this._activeItem);
		if (this._shouldActivationFollowFocus) this._activateCurrentItem();
	}
	...
	_findNextAvailableItemIndex(startingIndex) {
		for (let i = startingIndex + 1; i < this._items.length; i++) if (!this._skipPredicateFn(this._items[i])) return i;
		return startingIndex;
	}
	_findPreviousAvailableItemIndex(startingIndex) {
		for (let i = startingIndex - 1; i >= 0; i--) if (!this._skipPredicateFn(this._items[i])) return i;
		return startingIndex;
	}
	...
	_expandCurrentItem() {
		if (!this._activeItem) return;
		if (!this._isCurrentItemExpanded()) this._activeItem.expand();
		else coerceObservable(this._activeItem.getChildren()).pipe(take(1)).subscribe((children) => {
	...
	_expandAllItemsAtCurrentItemLevel() {
		...
		const parent = this._activeItem.getParent();
		let itemsToExpand;
		if (!parent) itemsToExpand = of(this._items.filter((item) => item.getParent() === null));
		else itemsToExpand = coerceObservable(parent.getChildren());
